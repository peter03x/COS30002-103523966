...
from searches import SearchDijkstra
...
BOX_TYPES = {
	...
	"F":{"description":'SWAMP',"colour":"LIGHT_RED"},
	"I":{"description":'BRIDGE',"colour":"YELLOW"},
...
class BoxWorld(object):
	...
	def __init__(self, numOfXBoxes, numOfYBoxes, windowWidth, windowHeight):
		...
		fastMonster = Agent(id=1, speedType="FAST", agentType="MONSTER")
  
		slowMonster = Agent(id=2, speedType="SLOW", agentType="MONSTER")

		fastHuman = Agent(id=3, speedType="FAST", agentType="HUMAN")

		slowHuman = Agent(id=4, speedType="SLOW", agentType="HUMAN")
  
		self.agents = [fastMonster, slowMonster, fastHuman, slowHuman]
	...
	def planPath(self, agent, limit):
		...
		self.paths[agent.id] = SearchDijkstra(self.navigationGraphs[agent.id], agent.startBox.index, agent.targetBox.index, limit)
